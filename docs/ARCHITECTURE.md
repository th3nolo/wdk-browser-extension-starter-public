...
`src/lib/background/pending-dapp-approvals.ts` serializes approval state transitions. Approval first persists a `claimed` record, then validates the connection and persists `executing` before calling the wallet execution boundary. Rejection, expiry, wallet cancellation, site revocation and connected account/network changes can cancel a claim before execution starts. After execution starts they cannot undo it or free its deduplication key; provider calls do not hold the storage queue. A changed or expired wallet session is checked again immediately before execution.

On a worker restart, interrupted claims are cancelled. Executions without a known result become `uncertain` and block approval and equivalent request retries, even after their original deadline. The caller is told to check the wallet/network; this starter does not infer that an RPC error means no broadcast, and does not automatically resubmit or reconcile unknown transactions. A late successful result or a result saved before a settlement failure can return the original hash/signature without executing again. Successful settlement also retains a receipt for one equivalent request in case response delivery is interrupted; that request receives the original result rather than creating another approval. These barriers use browser session storage, so they survive worker eviction but not browser-session storage loss. `controller.approval-races.test.ts` exercises controller requests with fake execution, storage failures, lifecycle races and restart recovery. `approval-bridge.integration.test.ts` runs the actual content and background entrypoints with the authenticated inpage provider in jsdom, using fake browser transport and wallet execution.
